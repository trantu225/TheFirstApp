package tiwaco.thefirstapp.DTO;

/**
 * Created by Admin on 18/6/2019.
 */

public class JSONTHANHTOANTHU {

    String BienLai;
    String ChiSoCu;
    String ChiSoMoi;
    String GhiChu;
    String TransactionID;
    String Lon;
    String MaKhachHang;
    String SLTieuThu;
    String maduong;
    String tiennuoc;
    String phi;
    String tongcong;
    String kyhd;
    String thue;
    String m3muc1;
    String m3muc2;
    String m3muc3;
    String m3muc4;
    String tienmuc1;
    String tienmuc2;
    String tienmuc3;
    String tienmuc4;
    String ngaythanhtoan;
    String ngaythanhtoanrequest;
    String capnhatthu;
    String nhanvienthu;
    String laninbiennhan;
    String laninthongbaosau;
    String laninthongbaotruoc;
    String thanhtoantamthu;

    public String getBIENLAI() {
        return BienLai;
    }

    public void setBIENLAI(String BIENLAI) {
        this.BienLai = BIENLAI;
    }

    public String getChiSoCu() {
        return ChiSoCu;
    }

    public void setChiSoCu(String chiSoCu) {
        ChiSoCu = chiSoCu;
    }

    public String getChiSoMoi() {
        return ChiSoMoi;
    }

    public void setChiSoMoi(String chiSoMoi) {
        ChiSoMoi = chiSoMoi;
    }

    public String getGhiChu() {
        return GhiChu;
    }

    public void setGhiChu(String ghiChu) {
        GhiChu = ghiChu;
    }

    public String getTRANSACTIONID() {
        return TransactionID;
    }

    public void setTRANSACTIONID(String TRANSACTIONID) {
        this.TransactionID = TRANSACTIONID;
    }

    public String getLon() {
        return Lon;
    }

    public void setLon(String lon) {
        Lon = lon;
    }

    public String getMaKhachHang() {
        return MaKhachHang;
    }

    public void setMaKhachHang(String maKhachHang) {
        MaKhachHang = maKhachHang;
    }

    public String getSLTieuThu() {
        return SLTieuThu;
    }

    public void setSLTieuThu(String SLTieuThu) {
        this.SLTieuThu = SLTieuThu;
    }

    public String getMaduong() {
        return maduong;
    }

    public void setMaduong(String maduong) {
        this.maduong = maduong;
    }

    public String getTiennuoc() {
        return tiennuoc;
    }

    public void setTiennuoc(String tiennuoc) {
        this.tiennuoc = tiennuoc;
    }

    public String getPhi() {
        return phi;
    }

    public void setPhi(String phi) {
        this.phi = phi;
    }

    public String getTongcong() {
        return tongcong;
    }

    public void setTongcong(String tongcong) {
        this.tongcong = tongcong;
    }

    public String getKyhd() {
        return kyhd;
    }

    public void setKyhd(String kyhd) {
        this.kyhd = kyhd;
    }

    public String getThue() {
        return thue;
    }

    public void setThue(String thue) {
        this.thue = thue;
    }

    public String getM3muc1() {
        return m3muc1;
    }

    public void setM3muc1(String m3muc1) {
        this.m3muc1 = m3muc1;
    }

    public String getM3muc2() {
        return m3muc2;
    }

    public void setM3muc2(String m3muc2) {
        this.m3muc2 = m3muc2;
    }

    public String getM3muc3() {
        return m3muc3;
    }

    public void setM3muc3(String m3muc3) {
        this.m3muc3 = m3muc3;
    }

    public String getM3muc4() {
        return m3muc4;
    }

    public void setM3muc4(String m3muc4) {
        this.m3muc4 = m3muc4;
    }

    public String getTienmuc1() {
        return tienmuc1;
    }

    public void setTienmuc1(String tienmuc1) {
        this.tienmuc1 = tienmuc1;
    }

    public String getTienmuc2() {
        return tienmuc2;
    }

    public void setTienmuc2(String tienmuc2) {
        this.tienmuc2 = tienmuc2;
    }

    public String getTienmuc3() {
        return tienmuc3;
    }

    public void setTienmuc3(String tienmuc3) {
        this.tienmuc3 = tienmuc3;
    }

    public String getTienmuc4() {
        return tienmuc4;
    }

    public void setTienmuc4(String tienmuc4) {
        this.tienmuc4 = tienmuc4;
    }

    public String getNgaythanhtoan() {
        return ngaythanhtoan;
    }

    public void setNgaythanhtoan(String ngaythanhtoan) {
        this.ngaythanhtoan = ngaythanhtoan;
    }

    public String getNgaythanhtoanrequest() {
        return ngaythanhtoanrequest;
    }

    public void setNgaythanhtoanrequest(String ngaythanhtoanrequest) {
        this.ngaythanhtoanrequest = ngaythanhtoanrequest;
    }

    public String getCapnhatthu() {
        return capnhatthu;
    }

    public void setCapnhatthu(String capnhatthu) {
        this.capnhatthu = capnhatthu;
    }

    public String getNhanvienthu() {
        return nhanvienthu;
    }

    public void setNhanvienthu(String nhanvienthu) {
        this.nhanvienthu = nhanvienthu;
    }

    public String getLaninbiennhan() {
        return laninbiennhan;
    }

    public void setLaninbiennhan(String laninbiennhan) {
        this.laninbiennhan = laninbiennhan;
    }

    public String getLaninthongbaosau() {
        return laninthongbaosau;
    }

    public void setLaninthongbaosau(String laninthongbaosau) {
        this.laninthongbaosau = laninthongbaosau;
    }

    public String getLaninthongbaotruoc() {
        return laninthongbaotruoc;
    }

    public void setLaninthongbaotruoc(String laninthongbaotruoc) {
        this.laninthongbaotruoc = laninthongbaotruoc;
    }

    public String getThanhtoantamthu() {
        return thanhtoantamthu;
    }

    public void setThanhtoantamthu(String thanhtoantamthu) {
        this.thanhtoantamthu = thanhtoantamthu;
    }
}
